...
export default function LandingPage() {
  ...
  return (
    <div style={{
      ...
      <nav style={{
        ...
        {isMobile ? (
          ...
          <div style={{ display: 'flex', gap: 28, alignItems: 'center' }}>
            {['Fitur', 'Use Case', 'Agen', 'Harga', 'FAQ'].map(item => (
        ...
        {isMobile && menuOpen && (
          <div style={{
            ...
            {['Fitur', 'Use Case', 'Agen', 'Harga', 'FAQ'].map(item => (
              <a key={item} href={`#${item.toLowerCase().replace(/ /g, '')}`} onClick={() => setMenuOpen(false)} style={{
      ...
      {/* Use Cases */}
      <section id="usecase" style={{ position: 'relative', zIndex: 1, padding: `80px ${pad}`, maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          <div style={{ fontSize: 10, color: '#00d4ff', textTransform: 'uppercase', letterSpacing: 3, marginBottom: 12, fontFamily: "'JetBrains Mono', monospace" }}>SIAPA YANG COCOK</div>
          <h2 style={{ fontSize: isMobile ? 28 : 36, fontWeight: 800, marginBottom: 12 }}>Dibuat Untuk Anda</h2>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.4)', maxWidth: 500, margin: '0 auto' }}>Apapun bisnis Anda, The Blue Shark punya agent yang tepat</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 20 }}>
          {[
            {
              icon: '🏪', title: 'UMKM & Toko Online', color: '#00e676',
              problems: 'Riset kompetitor mahal, bikin konten lama, gak paham data pelanggan',
              solutions: [
                '🔍 Riset pasar & analisis kompetitor instan',
                '✍️ Generate caption, deskripsi produk, & konten marketing',
                '📊 Pahami review pelanggan & temukan peluang',
              ],
              result: 'Hemat 10+ jam/minggu untuk riset & konten',
            },
            {
              icon: '🏢', title: 'Agency & Freelancer', color: '#00d4ff',
              problems: 'Handle banyak klien, deadline ketat, butuh output cepat & berkualitas',
              solutions: [
                '🚀 Strategi marketing siap presentasi per klien',
                '✍️ Copywriting & content plan dalam hitungan menit',
                '📊 Report analisis sentimen & performa brand',
              ],
              result: 'Handle 3x lebih banyak klien tanpa tambah tim',
            },
            {
              icon: '🚀', title: 'Startup & Founder', color: '#aa00ff',
              problems: 'Budget terbatas, tim kecil, butuh keputusan cepat berbasis data',
              solutions: [
                '🔍 Market sizing, TAM/SAM/SOM, & competitor mapping',
                '🛡️ Audit keamanan aplikasi & compliance check',
                '⚙️ Otomatisasi workflow tanpa hire engineer',
              ],
              result: 'Keputusan bisnis berbasis data, bukan asumsi',
            },
            {
              icon: '🎓', title: 'Mahasiswa & Peneliti', color: '#ffd600',
              problems: 'Riset data memakan waktu, sulit menganalisis tren, butuh referensi cepat',
              solutions: [
                '🔍 Analisis tren industri & data pasar terkini',
                '📊 Sentiment analysis untuk riset akademis',
                '🧠 ML Performance agent untuk proyek data science',
              ],
              result: 'Riset yang biasanya berminggu-minggu jadi hitungan jam',
            },
            {
              icon: '📱', title: 'Content Creator', color: '#ff6b35',
              problems: 'Kehabisan ide, susah konsisten, gak tau konten apa yang works',
              solutions: [
                '✍️ Ide konten unlimited + caption siap pakai',
                '📊 Analisis tren & sentiment topik yang lagi viral',
                '🚀 Strategi growth & optimasi engagement',
              ],
              result: 'Konten konsisten setiap hari tanpa creative block',
            },
            {
              icon: '🏭', title: 'Perusahaan & Enterprise', color: '#ff1744',
              problems: 'Butuh insight multi-departemen, keamanan data, & skalabilitas',
              solutions: [
                '🤖 Multi-Agent Collaboration untuk analisis 360°',
                '🛡️ Keamanan enterprise-grade & compliance',
                '💬 Customer support automation & knowledge base',
              ],
              result: 'Tim AI yang kerja 24/7 tanpa biaya overhead',
            },
          ].map(uc => (
            <div key={uc.title} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                ...
                <h3 style={{ fontSize: 16, fontWeight: 700, color: uc.color }}>{uc.title}</h3>
              </div>
              <div style={{
                fontSize: 12, color: 'rgba(255,255,255,0.35)', lineHeight: 1.6,
                padding: '10px 14px', background: 'rgba(255,255,255,0.02)',
                borderRadius: 10, marginBottom: 16,
                borderLeft: `2px solid ${uc.color}33`,
              }}>
                <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1 }}>Masalah:</span><br/>
                {uc.problems}
              </div>
              <div style={{ flex: 1, marginBottom: 16 }}>
                {uc.solutions.map((s, i) => (
                  <div key={i} style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6, marginBottom: 8 }}>{s}</div>
                ))}
              </div>
              <div style={{
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: uc.color }}>✦ {uc.result}</span>
              </div>
            </div>
          ))}
        </div>
        <div style={{ textAlign: 'center', marginTop: 40 }}>
          <a href="/" style={{
            display: 'inline-block', padding: isMobile ? '14px 32px' : '16px 40px', borderRadius: 14,
            background: 'linear-gradient(135deg, #00d4ff 0%, #0057ff 100%)',
            color: '#fff', textDecoration: 'none', fontSize: 16, fontWeight: 600,
            boxShadow: '0 8px 32px rgba(0,212,255,0.3)',
          }}>Coba Gratis — Temukan Agent Untuk Anda</a>
        </div>
      </section>
      ...
      <footer style={{ position: 'relative', zIndex: 1, borderTop: '1px solid rgba(255,255,255,0.06)', padding: isMobile ? '40px 16px 24px' : '60px 40px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '2fr 1fr 1fr 1fr', gap: isMobile ? 32 : 40, textAlign: isMobile ? 'center' : 'left', marginBottom: 40 }}>
          ...
          <div>
            ...
            {['Fitur', 'Use Case', 'Agen', 'Harga', 'FAQ'].map(item => (
              <a key={item} href={`#${item.toLowerCase().replace(/ /g, '')}`} style={{ display: 'block', fontSize: 13, color: 'rgba(255,255,255,0.4)', textDecoration: 'none', marginBottom: 10 }}
                onMouseEnter={e => e.currentTarget.style.color = '#00d4ff'}
