          ...
          {AGENTS.map(agent => (
            <button key={agent.id} onClick={() => setActiveAgent(agent)}
              style={{
                ...
                border: `1px solid ${activeAgent.id === agent.id ? `${agent.color}50` : 'rgba(255,255,255,0.06)'}`,
            ...
            >
              <div style={{
                ...
                background: activeAgent.id === agent.id ? agent.gradient : 'rgba(255,255,255,0.05)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 16, flexShrink: 0,
                boxShadow: activeAgent.id === agent.id ? `0 4px 16px ${agent.color}30` : 'none',
              }}>{agent.icon}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontSize: 12, fontWeight: 600,
                  color: activeAgent.id === agent.id ? agent.color : '#c0c8d4',
                }}>{agent.name}</div>
                <div style={{
                  fontSize: 9, color: 'rgba(255,255,255,0.3)',
                  fontFamily: "'JetBrains Mono', monospace",
        ...
        <div style={{ padding: '14px 20px 20px', background: 'linear-gradient(180deg, transparent 0%, rgba(6,13,26,0.8) 100%)' }}>
          <div style={{
            ...
            <textarea ref={inputRef} value={input}
              ...
              placeholder={activeAgent.placeholder} rows={1}
              style={{
            ...
            <button onClick={sendMessage} disabled={!input.trim() || isLoading}
              style={{
                ...
                background: input.trim() && !isLoading ? activeAgent.gradient : 'rgba(255,255,255,0.05)',
                border: 'none', cursor: input.trim() && !isLoading ? 'pointer' : 'default',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 17, color: '#fff', transition: 'all 0.3s ease', flexShrink: 0,
                boxShadow: input.trim() && !isLoading ? `0 4px 16px ${activeAgent.color}40` : 'none',
              }}
            >{isLoading ? '⏳' : '↑'}</button>
          </div>
          <div style={{
